...
def chord_diagram(mat, names=None, order=None, width=0.1, pad=2., gap=0.03,
    ...
    chord_colors : str, RGB tuple, list (default: None)
        Specify a color for each chord:
         * "red", all chords have this color
         * [1, 0, 0] : all chords have this RGB color (red)
         * ["red","green","blue"] : each chord gets this color.
    ...
    **kwargs : keyword arguments
    ...
    if chord_colors is None:
       chord_colors=colors
    elif isinstance(chord_colors,str):
        chord_colors=[chord_colors]*num_nodes
        print(chord_colors)
    else:
        try:
            chord_colors = [to_rgb(chord_colors)]*num_nodes
        except ValueError:
            assert len(chord_colors)==num_nodes, "If you pass a list of chord_colors, they have to have len %u"%num_nodes
